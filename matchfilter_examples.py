...
for i, fm in enumerate(filter_methods):
    ...
    title = r'Chirp, $0.8\,'+u'\u2013'+'\,2.2\,$MHz, $2\,\mu$s'
    ...
    if 0:
        ...
        bark2.plot2d(prodarray, MIN=0.8, MAX=0.9, cmap='inferno')
        ...
        bark2.plot2d(prodarray, MIN=0, MAX=1, cmap='inferno')
        # for n in range(N):
    ...
    title = r'Golay Complementary Code ($3$), $2\,$MHz'

    plotsC = golayC.filter_example(
        filter_method=gfilt1, trim=gtrim, title=title, x=x,
        plotresults=(0, 0, 0, 0, 0))
